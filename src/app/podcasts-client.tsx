  ...
  return (
    <>
      <section className="page-intro split-intro">
        ...
        <div className="catalog-count"><strong>{channels.length}</strong><span>podcast channels</span></div>
      </section>
      <section className="podcast-channel-strip">
        <div>
          <p className="kicker">Your channels</p>
          <div className="podcast-channel-list">
            {channels.map((channel) => <Link key={channel.id} href={appPath("/channels/" + channel.id)} className="podcast-channel-chip"><span>{channel.name.slice(0, 1)}</span>{channel.name}</Link>)}
          </div>
        </div>
